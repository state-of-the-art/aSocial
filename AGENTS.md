...
## Code Generation (protoc-gen-coredb)

CRUD operations, KV key helpers, validation functions, factory methods and the plugin permission
system are auto-generated from protobuf annotations by a custom protoc plugin.

### Pipeline

1. **Proto annotations** – each entity message in `libs/asocial_proto/proto/asocial/v1/` declares
   an `entity_config` option (key prefix, key pattern, scope, sort) and per-field `deps_config`
   options (foreign-key references).
2. **protoc-gen-coredb** (`libs/asocial_proto/libs/asocial_protogen/`) is compiled during the build
   and invoked by `protobuf_generate()`. It reads the annotations, builds a data model, and
   renders C++ code using the `asocial_tpl` template engine.
3. **Generated output** – `asocial/v1/coredb.gen.h` (one per proto file) is compiled into the
   `asocial_proto` shared library. It contains:
   * `asocial::v1::db::keys::*` – KV key constants and inline helper functions
   * `asocial::v1::db::validate<Entity>()` / `validate<Entity>Full()` – validation with FK checks
   * `asocial::v1::db::create<Entity>()` – factory functions (UID + timestamps pre-filled)
   * `asocial::v1::db::AccessCategory` / `AccessPermissions` – plugin permission enum and helpers
   * `asocial::v1::db::toTimestamp()`, `fromTimestamp()`, `newId()` – common utilities

### Template Engine (asocial_tpl)

`libs/asocial_tpl/` is a standalone Jinja2-like template engine using only Qt 6 Core. Features:

* Delimiters: `{{ expr }}`, `{% tag %}`, `{# comment #}`
* Control flow: `if`/`elif`/`else`/`endif`, `for`/`endfor`, `set`
* Expressions: dot access, filters (`|upper`, `|snake_case`, `|join`, ...), comparisons, logic
* Loop vars: `loop.index`, `loop.first`, `loop.last`, `loop.length`
* Context: `QVariantMap` with nested maps and lists

The engine is used by `protoc-gen-coredb` for code generation and is available to any part of the
application that needs text templating.
...
Factory methods (`db::create*`) return populated but **unsaved** objects;
callers must call the corresponding `store*` method to persist.

Each entity message carries an `entity_config` annotation defining its
KV key prefix, key pattern, scope, and sort order. Foreign-key fields
use `deps_config` annotations with an `fk` string (e.g. `'Persona.uid'`).
These annotations drive the `protoc-gen-coredb` code generator.

* `options_deps.proto` – `DepsConfig` (field FK), `EntityConfig` (message storage config)
* `coredb.gen.h` – Generated by `protoc-gen-coredb` (CMake runs one `protoc` with all
                   entity `.proto` files; the plugin option lists them plus an anchor file
                   that emits the combined header)
