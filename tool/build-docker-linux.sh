...
# Install the required build dependencies (tclsh - sqlcipher build):
sudo apt update; sudo DEBIAN_FRONTEND=noninteractive apt install -y gnupg2 tclsh
